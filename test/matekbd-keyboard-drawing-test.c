...
static const GOptionEntry options[] = {
    ...
    {"symbols", '\0', 0, G_OPTION_ARG_STRING, &symbols,
     ...
     "--symbols=us(pc104)+iso9995-3+group(switch)+ctrl(nocaps)",
     NULL},
...
static gboolean set_groups(gchar *groups_option,
                           MatekbdKeyboardDrawingGroupLevel *groupLevels) {
  MatekbdKeyboardDrawingGroupLevel *pgl = groupLevels;
  gint cntr, g;

  groupLevels[0].group = groupLevels[1].group = groupLevels[2].group =
      groupLevels[3].group = -1;

  if (groups_option == NULL) return TRUE;

  for (cntr = 4; --cntr >= 0;) {
    if (*groups_option == '\0') return FALSE;
    ...
    groups_option++;
    if (*groups_option == '\0') return TRUE;
    if (*groups_option != ',') return FALSE;
...
static gboolean set_levels(gchar *levels_option,
                           MatekbdKeyboardDrawingGroupLevel *groupLevels) {
  MatekbdKeyboardDrawingGroupLevel *pgl = groupLevels;
  gint cntr, l;
  gchar *p;

  groupLevels[0].level = groupLevels[1].level = groupLevels[2].level =
      groupLevels[3].level = -1;

  if (levels_option == NULL) return TRUE;

  for (cntr = 4; --cntr >= 0;) {
    if (*levels_option == '\0') return FALSE;

    l = (gint)strtol(levels_option, &p, 10) - 1;
    if (l < 0 || l >= 64) return FALSE;

    pgl->level = l;
    /* printf ("level %d\n", pgl->level); */

    levels_option = p;
    if (*levels_option == '\0') return TRUE;
    if (*levels_option != ',') return FALSE;
...
static void bad_keycode(MatekbdKeyboardDrawing *drawing, guint keycode) {
  g_warning(
      "got keycode %u, which is not on your keyboard according to your "
      "configuration",
      keycode);
}
...
gint main(gint argc, gchar **argv) {
  ...
  MatekbdKeyboardDrawingGroupLevel *pgroupLevels[4] = {
      &groupLevels[0], &groupLevels[1], &groupLevels[2], &groupLevels[3]};
  ...
  monitor =
      gdk_display_get_monitor_at_point(gdk_screen_get_display(screen), 0, 0);
  gdk_monitor_get_geometry(monitor, &rect);
  gtk_window_set_default_size(GTK_WINDOW(window), rect.width * 4 / 5,
                              rect.height * 1 / 2);
  ...
  matekbd_keyboard_drawing_set_groups_levels(
      MATEKBD_KEYBOARD_DRAWING(matekbd_keyboard_drawing), pgroupLevels);
  ...
  if (symbols || geometry || keycodes) {
    ...
    if (symbols)
      ...
      names.symbols = (gchar *)matekbd_keyboard_drawing_get_symbols(
          MATEKBD_KEYBOARD_DRAWING(matekbd_keyboard_drawing));

    if (keycodes)
      names.keycodes = keycodes;
    else
      names.keycodes = (gchar *)matekbd_keyboard_drawing_get_keycodes(
          MATEKBD_KEYBOARD_DRAWING(matekbd_keyboard_drawing));

    if (geometry)
      names.geometry = geometry;
    else
      names.geometry = (gchar *)matekbd_keyboard_drawing_get_geometry(
          MATEKBD_KEYBOARD_DRAWING(matekbd_keyboard_drawing));

    success = matekbd_keyboard_drawing_set_keyboard(
        MATEKBD_KEYBOARD_DRAWING(matekbd_keyboard_drawing), &names);
    if (!success) {
      g_printerr(
