...
static void matekbd_log_appender(const char file[], const char function[],
                                 int level, const char format[], va_list args) {
  time_t now = time(NULL);
  g_log(NULL, G_LOG_LEVEL_DEBUG, "[%08ld,%03d,%s:%s/] \t", (long)now, level,
        file, function);
  g_logv(NULL, G_LOG_LEVEL_DEBUG, format, args);
}

void matekbd_install_glib_log_appender(void) {
 ...
 */
GdkRectangle *matekbd_preview_load_position(void) {
...
void matekbd_preview_save_position(GdkRectangle *rect) {
  GSettings *settings = g_settings_new(MATEKBD_PREVIEW_CONFIG_SCHEMA);
 ...
 * matekbd_strv_append:
 *
 * Returns: (transfer full) (array zero-terminated=1): Append string to strv
 * array
 */
gchar **matekbd_strv_append(gchar **arr, gchar *element) {
