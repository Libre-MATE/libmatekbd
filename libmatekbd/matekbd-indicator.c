...
static void matekbd_indicator_global_init(void);
static void matekbd_indicator_global_term(void);
static GtkWidget *matekbd_indicator_prepare_drawing(MatekbdIndicator *gki,
                                                    guint group);
static void matekbd_indicator_set_current_page_for_group(MatekbdIndicator *gki,
                                                         guint group);
static void matekbd_indicator_set_current_page(MatekbdIndicator *gki);
static void matekbd_indicator_cleanup(MatekbdIndicator *gki);
static void matekbd_indicator_fill(MatekbdIndicator *gki);
static void matekbd_indicator_set_tooltips(MatekbdIndicator *gki,
                                           const char *str);

void matekbd_indicator_load_images() {
  int i;
  ...
  if (!globals.ind_cfg.show_flags) return;
  ...
  for (i = (int)max_num_groups; --i >= 0;
       image_filename = image_filename->next) {
    GdkPixbuf *image = NULL;
    char *image_file = (char *)image_filename->data;

    if (image_file != NULL) {
      GError *gerror = NULL;
      image = gdk_pixbuf_new_from_file(image_file, &gerror);
      if (image == NULL) {
      ...
      xkl_debug(150, "Image %d[%s] loaded -> %p[%dx%d]\n", i, image_file, image,
...
static void matekbd_indicator_update_images(void) {
  matekbd_indicator_free_images();
  matekbd_indicator_load_images();
}

void matekbd_indicator_set_tooltips(MatekbdIndicator *gki, const char *str) {
...
void matekbd_indicator_cleanup(MatekbdIndicator *gki) {
  int i;
...
void matekbd_indicator_fill(MatekbdIndicator *gki) {
  guint grp;
  ...
  for (grp = 0; grp < total_groups; grp++) {
    ...
    if (page == NULL) page = gtk_label_new("");
...
static gboolean matekbd_indicator_key_pressed(GtkWidget *widget,
                                              GdkEventKey *event,
...
static gboolean matekbd_indicator_button_pressed(GtkWidget *widget,
...
static void draw_flag(GtkWidget *flag, cairo_t *cr, GdkPixbuf *image) {
...
gchar *matekbd_indicator_extract_layout_name(guint group, XklEngine *engine,
                                             MatekbdKeyboardConfig *kbd_cfg,
                                             gchar **short_group_names,
                                             gchar **full_group_names) {
  ...
  if (group < g_strv_length(short_group_names)) {
    if (xkl_engine_get_features(engine) & XKLF_MULTIPLE_LAYOUTS_SUPPORTED) {
      ...
      char *variant_name;
      if (!matekbd_keyboard_config_split_items(full_layout_name, &layout_name,
                                               &variant_name))
        /* just in case */
      ...
      if (short_group_names != NULL) {
        ...
        if (short_group_name != NULL && *short_group_name != '\0') {
          /* drop the long name */
          g_free(layout_name);
          layout_name = g_strdup(short_group_name);
        }
      }
    } else {
  ...
  if (layout_name == NULL) layout_name = g_strdup("");

  return layout_name;
}

gchar *matekbd_indicator_create_label_title(guint group,
  ...
  if (g_hash_table_lookup_extended(*ln2cnt_map, layout_name,
  ...
  g_hash_table_insert(*ln2cnt_map, layout_name, GINT_TO_POINTER(counter + 1));
  return lbl_title;
}
...
static void matekbd_indicator_update_tooltips(MatekbdIndicator *gki) {
...
static void matekbd_indicator_parent_set(GtkWidget *gki,
                                         GtkWidget *previous_parent) {
  matekbd_indicator_update_tooltips(MATEKBD_INDICATOR(gki));
}

void matekbd_indicator_reinit_ui(MatekbdIndicator *gki) {
...
static void matekbd_indicator_cfg_changed(GSettings *settings, gchar *key,
                                          gpointer user_data) {
  xkl_debug(100, "General configuration changed in GSettings - reiniting...\n");
  matekbd_desktop_config_load_from_gsettings(&globals.cfg);
  matekbd_desktop_config_activate(&globals.cfg);
  ForAllIndicators() { matekbd_indicator_reinit_ui(gki); }
  NextIndicator();
}

/* Should be called once for all widgets */
static void matekbd_indicator_ind_cfg_changed(GSettings *settings, gchar *key,
                                              gpointer user_data) {
  xkl_debug(100, "Applet configuration changed in GSettings - reiniting...\n");
...
static void matekbd_indicator_load_group_names(const gchar **layout_ids,
  ...
  if (!matekbd_desktop_config_load_group_descriptions(
    ...
    gint i;
    guint total_groups = xkl_engine_get_num_groups(globals.engine);
    globals.full_group_names = g_new0(gchar *, (gsize)(total_groups + 1));
...
/* Should be called once for all widgets */
static void matekbd_indicator_kbd_cfg_callback(MatekbdIndicator *gki) {
...
static void matekbd_indicator_state_callback(XklEngine *engine,
...
void matekbd_indicator_set_current_page(MatekbdIndicator *gki) {
  XklState *cur_state;
...
void matekbd_indicator_set_current_page_for_group(MatekbdIndicator *gki,
                                                  guint group) {
...
/* Should be called once for all widgets */
static GdkFilterReturn matekbd_indicator_filter_x_evt(GdkXEvent *xev,
                                                      GdkEvent *event) {
  ...
  switch (xevent->type) {
    case ReparentNotify: {
      ...
      ForAllIndicators() {
        GdkWindow *w = gtk_widget_get_parent_window(GTK_WIDGET(gki));
...
/* Should be called once for all widgets */
static void matekbd_indicator_start_listen(void) {
  gdk_window_add_filter(NULL, (GdkFilterFunc)matekbd_indicator_filter_x_evt,
                        NULL);
...
/* Should be called once for all widgets */
static void matekbd_indicator_stop_listen(void) {
  xkl_engine_stop_listen(globals.engine, XKLL_TRACK_KEYBOARD_STATE);

  gdk_window_remove_filter(NULL, (GdkFilterFunc)matekbd_indicator_filter_x_evt,
                           NULL);
  gdk_window_remove_filter(gdk_get_default_root_window(),
                           (GdkFilterFunc)matekbd_indicator_filter_x_evt, NULL);
}

static gboolean matekbd_indicator_scroll(GtkWidget *gki,
...
static void matekbd_indicator_init(MatekbdIndicator *gki) {
  ...
  if (globals.engine == NULL) {
    matekbd_indicator_set_tooltips(gki, _("XKB initialization error"));
    return;
  }
...
static void matekbd_indicator_finalize(GObject *obj) {
  ...
  xkl_debug(100, "The instance of mate-kbd-indicator successfully finalized\n");
...
static void matekbd_indicator_class_init(MatekbdIndicatorClass *klass) {
  ...
  /* Signals */
  g_signal_new("reinit-ui", MATEKBD_TYPE_INDICATOR, G_SIGNAL_RUN_LAST,
               G_STRUCT_OFFSET(MatekbdIndicatorClass, reinit_ui), NULL, NULL,
               matekbd_indicator_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void matekbd_indicator_global_init(void) {
  ...
  g_signal_connect(globals.engine, "X-state-changed",
                   G_CALLBACK(matekbd_indicator_state_callback), NULL);
  g_signal_connect(globals.engine, "X-config-changed",
                   G_CALLBACK(matekbd_indicator_kbd_cfg_callback), NULL);
  ...
  matekbd_desktop_config_start_listen(
      &globals.cfg, (GCallback)matekbd_indicator_cfg_changed, NULL);
  matekbd_indicator_config_start_listen(
      &globals.ind_cfg, (GCallback)matekbd_indicator_ind_cfg_changed, NULL);
...
GtkWidget *matekbd_indicator_new(void) {
  return GTK_WIDGET(g_object_new(matekbd_indicator_get_type(), NULL));
}

void matekbd_indicator_set_parent_tooltips(MatekbdIndicator *gki,
                                           gboolean spt) {
  gki->priv->set_parent_tooltips = spt;
  matekbd_indicator_update_tooltips(gki);
}

void matekbd_indicator_set_tooltips_format(const gchar format[]) {
  globals.tooltips_format = format;
  ForAllIndicators() matekbd_indicator_update_tooltips(gki);
  NextIndicator()
}
 ...
 */
XklEngine *matekbd_indicator_get_xkl_engine() { return globals.engine; }
 ...
 */
gchar **matekbd_indicator_get_group_names() { return globals.full_group_names; }

gchar *matekbd_indicator_get_image_filename(guint group) {
...
void matekbd_indicator_set_angle(MatekbdIndicator *gki, gdouble angle) {
