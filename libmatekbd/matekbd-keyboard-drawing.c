...
enum { BAD_KEYCODE = 0, NUM_SIGNALS };
...
static gint xkb_to_pixmap_coord(MatekbdKeyboardDrawingRenderContext *context,
                                gint n) {
  return (gint)(((gdouble)n) * context->scale_numerator /
                context->scale_denominator);
}

static gdouble xkb_to_pixmap_double(
    MatekbdKeyboardDrawingRenderContext *context, gdouble d) {
...
static void rotate_coordinate(gint origin_x, gint origin_y, gint x, gint y,
                              gdouble angle, gint *rotated_x, gint *rotated_y) {
  *rotated_x =
      origin_x + (gint)((cos(M_PI * angle / 1800.0) * (double)(x - origin_x)) -
                        (sin(M_PI * angle / 1800.0) * (double)(y - origin_y)));
  *rotated_y =
      origin_y + (gint)((sin(M_PI * angle / 1800.0) * (double)(x - origin_x)) +
                        (cos(M_PI * angle / 1800.0) * (double)(y - origin_y)));
}

static gdouble length(gdouble x, gdouble y) { return sqrt(x * x + y * y); }

static gdouble point_line_distance(gdouble ax, gdouble ay, gdouble nx,
                                   gdouble ny) {
  return ax * nx + ay * ny;
}

static void normal_form(gdouble ax, gdouble ay, gdouble bx, gdouble by,
...
static void inverse(gdouble a, gdouble b, gdouble c, gdouble d, gdouble *e,
                    gdouble *f, gdouble *g, gdouble *h) {
  gdouble det;
...
static void multiply(gdouble a, gdouble b, gdouble c, gdouble d, gdouble e,
                     gdouble f, gdouble *x, gdouble *y) {
  *x = a * e + b * f;
  *y = c * e + d * f;
}

static void intersect(gdouble n1x, gdouble n1y, gdouble d1, gdouble n2x,
                      gdouble n2y, gdouble d2, gdouble *x, gdouble *y) {
  gdouble e, f, g, h;
 ...
 */
static void rounded_corner(cairo_t *cr, gdouble bx, gdouble by, gdouble cx,
                           gdouble cy, gdouble radius) {
  gdouble ax, ay;
  gdouble n1x, n1y, d1;
  gdouble n2x, n2y, d2;
  gdouble pd1, pd2;
...
static void rounded_polygon(cairo_t *cr, gboolean filled, gdouble radius,
                            GdkPoint *points, gint num_points) {
  gint i, j;

  cairo_move_to(cr, (gdouble)(points[num_points - 1].x + points[0].x) / 2,
...
static void draw_polygon(MatekbdKeyboardDrawingRenderContext *context,
                         GdkRGBA *fill_color, gint xkb_x, gint xkb_y,
                         XkbPointRec *xkb_points, guint num_points,
                         gdouble radius) {
  GdkPoint *points;
  gboolean filled;
  gint i;
  ...
  rounded_polygon(context->cr, filled, xkb_to_pixmap_double(context, radius),
                  points, num_points);

  g_free(points);
}

static void curve_rectangle(cairo_t *cr, gdouble x0, gdouble y0, gdouble width,
...
static void draw_curve_rectangle(cairo_t *cr, gboolean filled,
                                 GdkRGBA *fill_color, gint x, gint y,
                                 gint width, gint height, gdouble radius) {
...
static void draw_rectangle(MatekbdKeyboardDrawingRenderContext *context,
                           GdkRGBA *fill_color, gint angle, gint xkb_x,
                           gint xkb_y, gint xkb_width, gint xkb_height,
                           gdouble radius) {
  if (angle == 0) {
    ...
    width = xkb_to_pixmap_coord(context, xkb_x + xkb_width) - x;
    height = xkb_to_pixmap_coord(context, xkb_y + xkb_height) - y;

    draw_curve_rectangle(context->cr, filled, fill_color, x, y, width, height,
    ...
    points[0].x = xkb_x;
    points[0].y = xkb_y;
    rotate_coordinate(xkb_x, xkb_y, xkb_x + xkb_width, xkb_y, angle, &x, &y);
    points[1].x = x;
    points[1].y = y;
    rotate_coordinate(xkb_x, xkb_y, xkb_x + xkb_width, xkb_y + xkb_height,
                      angle, &x, &y);
    points[2].x = x;
    points[2].y = y;
    rotate_coordinate(xkb_x, xkb_y, xkb_x, xkb_y + xkb_height, angle, &x, &y);
    points[3].x = x;
    points[3].y = y;

    /* the points we've calculated are relative to 0,0 */
    draw_polygon(context, fill_color, 0, 0, points, 4, radius);
  }
}

static void draw_outline(MatekbdKeyboardDrawingRenderContext *context,
  ...
  if (outline->num_points == 1) {
    if (color)
      draw_rectangle(context, color, angle, origin_x, origin_y,
    ...
    rotate_coordinate(origin_x, origin_y, origin_x + outline->points[0].x,
                      origin_y + outline->points[0].y, angle, &rotated_x0,
                      &rotated_y0);
    if (color)
      draw_rectangle(context, color, angle, rotated_x0, rotated_y0,
                     outline->points[1].x, outline->points[1].y,
                     (gdouble)outline->corner_radius);

    draw_rectangle(context, NULL, angle, rotated_x0, rotated_y0,
                   outline->points[1].x, outline->points[1].y,
                   (gdouble)outline->corner_radius);
  } else {
    if (color)
      draw_polygon(context, color, origin_x, origin_y, outline->points,
                   outline->num_points, outline->corner_radius);

    draw_polygon(context, NULL, origin_x, origin_y, outline->points,
                 outline->num_points, outline->corner_radius);
  }
}

/* see PSColorDef in xkbprint */
static gboolean parse_xkb_color_spec(gchar *colorspec, GdkRGBA *color) {
  glong level;
...
static guint find_keycode(MatekbdKeyboardDrawing *drawing, gchar *key_name) {
...
static void set_markup(MatekbdKeyboardDrawingRenderContext *context,
                       gchar *txt) {
  PangoLayout *layout = context->layout;
  txt = strcmp("<", txt) ? txt : "&lt;";
  txt = strcmp("&", txt) ? txt : "&amp;";
  if (g_utf8_strlen(txt, -1) > 1) {
    gchar *buf = g_strdup_printf("<span size=\"xx-small\">%s</span>", txt);
...
static void set_key_label_in_layout(
    MatekbdKeyboardDrawingRenderContext *context, guint keyval) {
  gchar buf[5];
  gunichar uc;
...
static void draw_pango_layout(MatekbdKeyboardDrawingRenderContext *context,
                              MatekbdKeyboardDrawing *drawing, gint angle,
                              gint x, gint y) {
  PangoLayout *layout = context->layout;
  GdkRGBA *color;
  ...
  color = drawing->colors +
          (drawing->xkb->geom->label_color - drawing->xkb->geom->colors);
  ...
  i = 0;
  y_off = 0;
  for (line = pango_layout_get_line(layout, i); line != NULL;
    ...
    y_off += line_extents.height + pango_layout_get_spacing(layout);
  }
...
static void draw_key_label_helper(MatekbdKeyboardDrawingRenderContext *context,
                                  MatekbdKeyboardDrawing *drawing,
                                  KeySym keysym, gint angle,
  ...
  switch (glp) {
    ...
    case MATEKBD_KEYBOARD_DRAWING_POS_BOTTOMLEFT: {
      ycell = glp == MATEKBD_KEYBOARD_DRAWING_POS_BOTTOMLEFT;

      rotate_coordinate(x, y, x + padding,
                        y + padding + (height - 2 * padding) * ycell * 4 / 7,
    ...
    case MATEKBD_KEYBOARD_DRAWING_POS_BOTTOMRIGHT: {
      ycell = glp == MATEKBD_KEYBOARD_DRAWING_POS_BOTTOMRIGHT;

      rotate_coordinate(x, y, x + padding + (width - 2 * padding) * 4 / 7,
                        y + padding + (height - 2 * padding) * ycell * 4 / 7,
                        angle, &label_x, &label_y);
      label_max_width =
          PANGO_SCALE * ((width - 2 * padding) - (width - 2 * padding) * 4 / 7);
      break;
    }
    default:
      return;
  ...
  label_y -=
      (pango_layout_get_line_count(context->layout) - 1) *
      (pango_font_description_get_size(context->font_desc) / PANGO_SCALE);
...
static void draw_key_label(MatekbdKeyboardDrawingRenderContext *context,
                           MatekbdKeyboardDrawing *drawing, guint keycode,
                           gint angle, gint xkb_origin_x, gint xkb_origin_y,
                           gint xkb_width, gint xkb_height) {
  gint x, y, width, height;
  gint padding;
  gint g, l, glp;
  ...
  padding = (gint)(23.0 * context->scale_numerator /
                   context->scale_denominator); /* 2.3mm */

  x = xkb_to_pixmap_coord(context, xkb_origin_x);
  y = xkb_to_pixmap_coord(context, xkb_origin_y);
  width = xkb_to_pixmap_coord(context, xkb_origin_x + xkb_width) - x;
  height = xkb_to_pixmap_coord(context, xkb_origin_y + xkb_height) - y;

  for (glp = MATEKBD_KEYBOARD_DRAWING_POS_TOPLEFT;
       glp < MATEKBD_KEYBOARD_DRAWING_POS_TOTAL; glp++) {
    if (drawing->groupLevels[glp] == NULL) continue;
    g = drawing->groupLevels[glp]->group;
    l = drawing->groupLevels[glp]->level;

    if (g < 0 || g >= XkbKeyNumGroups(drawing->xkb, keycode)) continue;
    if (l < 0 || l >= XkbKeyGroupWidth(drawing->xkb, keycode, g)) continue;

    /* Skip "exotic" levels like the "Ctrl" level in PC_SYSREQ */
    if (l > 0) {
      guint mods = XkbKeyKeyType(drawing->xkb, keycode, g)->mods.mask;
      if ((mods & (ShiftMask | drawing->l3mod)) == 0) continue;
    }
    ...
    if (drawing->track_modifiers) {
      ...
      if (XkbTranslateKeyCode(drawing->xkb, keycode,
                              XkbBuildCoreState(drawing->mods, g), &mods_rtrn,
                              &keysym)) {
        draw_key_label_helper(context, drawing, keysym, angle, glp, x, y, width,
                              height, padding);
        /* reverse y order */
      }
    } else {
...
/*
 * The x offset is calculated for complex shapes. It is the rightmost of the
...
/* groups are from 0-3 */
static void draw_key(MatekbdKeyboardDrawingRenderContext *context,
                     MatekbdKeyboardDrawing *drawing,
...
static void invalidate_region(MatekbdKeyboardDrawing *drawing, gdouble angle,
                              gint origin_x, gint origin_y,
                              XkbShapeRec *shape) {
  GdkPoint points[4];
  ...
  points[1].x = xx;
  points[1].y = yy;
  rotate_coordinate(0, 0, shape->bounds.x2, shape->bounds.y2, angle, &xx, &yy);
  points[2].x = xx;
  points[2].y = yy;
  ...
  width = xkb_to_pixmap_coord(drawing->renderContext, x_max - x_min) + 12;
  height = xkb_to_pixmap_coord(drawing->renderContext, y_max - y_min) + 12;

  gtk_widget_queue_draw_area(GTK_WIDGET(drawing), x, y, width, height);
}

static void invalidate_indicator_doodad_region(
    MatekbdKeyboardDrawing *drawing, MatekbdKeyboardDrawingDoodad *doodad) {
  if (!drawing->xkb) return;

  invalidate_region(
      drawing, doodad->angle, doodad->origin_x + doodad->doodad->indicator.left,
      doodad->origin_y + doodad->doodad->indicator.top,
      &drawing->xkb->geom->shapes[doodad->doodad->indicator.shape_ndx]);
}

static void invalidate_key_region(MatekbdKeyboardDrawing *drawing,
                                  MatekbdKeyboardDrawingKey *key) {
  if (!drawing->xkb) return;

  invalidate_region(drawing, key->angle, key->origin_x, key->origin_y,
                    &drawing->xkb->geom->shapes[key->xkbkey->shape_ndx]);
}

static void draw_text_doodad(MatekbdKeyboardDrawingRenderContext *context,
                             MatekbdKeyboardDrawing *drawing,
  ...
  x = xkb_to_pixmap_coord(context, doodad->origin_x + text_doodad->left);
  y = xkb_to_pixmap_coord(context, doodad->origin_y + text_doodad->top);

  set_markup(context, text_doodad->text);
  draw_pango_layout(context, drawing, doodad->angle, x, y);
}

static void draw_indicator_doodad(MatekbdKeyboardDrawingRenderContext *context,
                                  MatekbdKeyboardDrawing *drawing,
                                  MatekbdKeyboardDrawingDoodad *doodad,
  ...
  color = drawing->colors + (doodad->on ? indicator_doodad->on_color_ndx
                                        : indicator_doodad->off_color_ndx);

  for (i = 0; i < 1; i++)
    draw_outline(context, shape->outlines + i, color, doodad->angle,
                 doodad->origin_x + indicator_doodad->left,
                 doodad->origin_y + indicator_doodad->top);
}

static void draw_shape_doodad(MatekbdKeyboardDrawingRenderContext *context,
                              MatekbdKeyboardDrawing *drawing,
                              MatekbdKeyboardDrawingDoodad *doodad,
  ...
  draw_outline(context, shape->primary ? shape->primary : shape->outlines,
               color, doodad->angle, doodad->origin_x + shape_doodad->left,
  ...
  for (i = 0; i < shape->num_outlines; i++) {
    if (shape->outlines + i == shape->approx ||
      ...
      continue;
    draw_outline(context, shape->outlines + i, NULL, doodad->angle,
                 doodad->origin_x + shape_doodad->left,
                 doodad->origin_y + shape_doodad->top);
  }
}

static void draw_doodad(MatekbdKeyboardDrawingRenderContext *context,
                        MatekbdKeyboardDrawing *drawing,
                        MatekbdKeyboardDrawingDoodad *doodad) {
  switch (doodad->doodad->any.type) {
    case XkbOutlineDoodad:
    case XkbSolidDoodad:
      draw_shape_doodad(context, drawing, doodad, &doodad->doodad->shape);
      break;

    case XkbTextDoodad:
      draw_text_doodad(context, drawing, doodad, &doodad->doodad->text);
      break;
...
static void redraw_overlapping_doodads(
  ...
  for (list = drawing->keyboard_items; list; list = list->next) {
    ...
    if (do_draw && item->type == MATEKBD_KEYBOARD_DRAWING_ITEM_TYPE_DOODAD)
      draw_doodad(context, drawing, (MatekbdKeyboardDrawingDoodad *)item);

    if (list->data == key) do_draw = TRUE;
  }
}

static void draw_keyboard_item(MatekbdKeyboardDrawingItem *item,
  ...
  switch (item->type) {
    ...
    case MATEKBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY_EXTRA:
      draw_key(context, drawing, (MatekbdKeyboardDrawingKey *)item);
      break;
...
static void draw_keyboard_to_context(
    MatekbdKeyboardDrawingRenderContext *context,
    MatekbdKeyboardDrawing *drawing) {
...
static gboolean create_cairo(MatekbdKeyboardDrawing *drawing) {
  GtkStyleContext *style_context = NULL;
  GtkStateFlags state;
  GdkRGBA dark_color;

  if (drawing == NULL) return FALSE;
  if (drawing->surface == NULL) return FALSE;
  ...
  gtk_style_context_get_background_color(style_context, state, &dark_color);
...
static void destroy_cairo(MatekbdKeyboardDrawing *drawing) {
  cairo_destroy(drawing->renderContext->cr);
  drawing->renderContext->cr = NULL;
}

static void draw_keyboard(MatekbdKeyboardDrawing *drawing) {
  GtkStyleContext *context = gtk_widget_get_style_context(GTK_WIDGET(drawing));
  ...
  drawing->surface = gdk_window_create_similar_surface(
      gtk_widget_get_window(GTK_WIDGET(drawing)), CAIRO_CONTENT_COLOR,
...
static void alloc_render_context(MatekbdKeyboardDrawing *drawing) {
  ...
  gtk_style_context_get(style_context,
                        ...
                        GTK_STYLE_PROPERTY_FONT, &context->font_desc, NULL);
...
static void free_render_context(MatekbdKeyboardDrawing *drawing) {
  MatekbdKeyboardDrawingRenderContext *context = drawing->renderContext;
...
static gboolean idle_redraw(gpointer user_data) {
...
static gboolean context_setup_scaling(
    MatekbdKeyboardDrawingRenderContext *context,
    MatekbdKeyboardDrawing *drawing, gdouble width, gdouble height,
  ...
  pango_font_description_set_size(
      context->font_desc,
      (gint)(72.0 * KEY_FONT_SIZE * dpi_x * context->scale_numerator /
             context->scale_denominator));
...
static void size_allocate(GtkWidget *widget, GtkAllocation *allocation,
  ...
  if (!context_setup_scaling(context, drawing, allocation->width,
                             allocation->height, 50, 50))
    return;
...
static gint key_event(GtkWidget *widget, GdkEventKey *event,
  ...
  if (event->hardware_keycode > drawing->xkb->max_key_code ||
      ...
      key->xkbkey == NULL) {
    g_signal_emit(drawing, matekbd_keyboard_drawing_signals[BAD_KEYCODE], 0,
...
static gboolean unpress_keys(MatekbdKeyboardDrawing *drawing) {
  gint i;
  ...
  if (create_cairo(drawing)) {
    for (i = drawing->xkb->min_key_code; i <= drawing->xkb->max_key_code; i++)
      if (drawing->keys[i].pressed) {
        ...
        draw_key(drawing->renderContext, drawing, drawing->keys + i);
...
static gint focus_event(GtkWidget *widget, GdkEventFocus *event,
                        MatekbdKeyboardDrawing *drawing) {
...
static gint compare_keyboard_item_priorities(MatekbdKeyboardDrawingItem *a,
...
static void init_indicator_doodad(MatekbdKeyboardDrawing *drawing,
  ...
  if (xkbdoodad->any.type == XkbIndicatorDoodad) {
    ...
    unsigned long phys_indicators = drawing->xkb->indicators->phys_indicators;
    ...
    for (index = 0; index < XkbNumIndicators; index++) {
      ...
      if (iname == sname && (phys_indicators & (1 << index))) break;
      if (iname == 0) break;
    }
    if (iname == 0)
      ...
      if (!XkbGetNamedIndicator(drawing->display, sname, NULL, &doodad->on,
                                NULL, NULL))
        doodad->on = 0;
    }
  }
...
static void init_keys_and_doodads(MatekbdKeyboardDrawing *drawing) {
  gint i, j, k;
  gint x, y;
    ...
    for (j = 0; j < section->num_rows; j++) {
      ...
      for (k = 0; k < row->num_keys; k++) {
        ...
        if (keycode >= drawing->xkb->min_key_code &&
            keycode <= drawing->xkb->max_key_code) {
          key = drawing->keys + keycode;
          if (key->type == MATEKBD_KEYBOARD_DRAWING_ITEM_TYPE_INVALID) {
            key->type = MATEKBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY;
          } else {
            /* duplicate key for the same keycode,
               already defined as MATEKBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY */
            key = g_new0(MatekbdKeyboardDrawingKey, 1);
            key->type = MATEKBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY_EXTRA;
          }
        } else {
          g_warning("key %4.4s: keycode = %u; not in range %d..%d\n",
                    xkbkey->name.name, keycode, drawing->xkb->min_key_code,
  ...
  drawing->keyboard_items = g_list_sort(
      drawing->keyboard_items, (GCompareFunc)compare_keyboard_item_priorities);
}

static void init_colors(MatekbdKeyboardDrawing *drawing) {
  gboolean result;
  gint i;
  ...
  for (i = 0; i < drawing->xkb->geom->num_colors; i++) {
    result = parse_xkb_color_spec(drawing->xkb->geom->colors[i].spec,
                                  drawing->colors + i);
...
static void free_cdik(/*colors doodads indicators keys */
                      MatekbdKeyboardDrawing *drawing) {
  GList *itemp;
...
static void alloc_cdik(MatekbdKeyboardDrawing *drawing) {
  if (!drawing->xkb) return;

  drawing->physical_indicators_size =
      drawing->xkb->indicators->phys_indicators + 1;
  drawing->physical_indicators =
      g_new0(MatekbdKeyboardDrawingDoodad *, drawing->physical_indicators_size);
  drawing->keys =
      g_new0(MatekbdKeyboardDrawingKey, drawing->xkb->max_key_code + 1);
}

static void process_indicators_state_notify(XkbIndicatorNotifyEvent *iev,
                                            MatekbdKeyboardDrawing *drawing) {
  ...
  for (i = 0; i <= drawing->xkb->indicators->phys_indicators; i++)
    if (drawing->physical_indicators[i] != NULL && (iev->changed & 1 << i)) {
      ...
      if ((state && !drawing->physical_indicators[i]->on) ||
        ...
        if (create_cairo(drawing)) {
          draw_doodad(drawing->renderContext, drawing,
...
static GdkFilterReturn xkb_state_notify_event_filter(
    GdkXEvent *gdkxev, GdkEvent *event, MatekbdKeyboardDrawing *drawing) {
#define modifier_change_mask                                           \
  (XkbModifierStateMask | XkbModifierBaseMask | XkbModifierLatchMask | \
   XkbModifierLockMask)

  if (!drawing->xkb) return GDK_FILTER_CONTINUE;
    ...
    switch (kev->any.xkb_type) {
      case XkbStateNotify:
        if (((kev->state.changed & modifier_change_mask) &&
          ...
          free_cdik(drawing);
          if (drawing->track_modifiers)
            matekbd_keyboard_drawing_set_mods(drawing, kev->state.compat_state);
          drawing->keys =
              g_new0(MatekbdKeyboardDrawingKey, drawing->xkb->max_key_code + 1);
      ...
      case XkbIndicatorStateNotify: {
        process_indicators_state_notify(&((XkbEvent *)gdkxev)->indicators,
                                        drawing);
      } break;
      ...
      case XkbNewKeyboardNotify: {
        ...
        if (drawing->track_modifiers)
          matekbd_keyboard_drawing_set_mods(drawing, state.compat_state);
        if (drawing->track_config)
          matekbd_keyboard_drawing_set_keyboard(drawing, NULL);
      } break;
    }
  }
...
static void destroy(MatekbdKeyboardDrawing *drawing) {
  free_render_context(drawing);
  gdk_window_remove_filter(NULL, (GdkFilterFunc)xkb_state_notify_event_filter,
                           drawing);
...
static void style_changed(MatekbdKeyboardDrawing *drawing) {
  pango_layout_context_changed(drawing->renderContext->layout);
}

static void matekbd_keyboard_drawing_init(MatekbdKeyboardDrawing *drawing) {
  ...
  if (gtk_widget_has_screen(GTK_WIDGET(drawing)))
    drawing->screen_num = gdk_x11_screen_get_screen_number(
        gtk_widget_get_screen(GTK_WIDGET(drawing)));
  else
    drawing->screen_num =
  ...
  drawing->l3mod =
      XkbKeysymToModifiers(drawing->display, GDK_KEY_ISO_Level3_Shift);

  drawing->xkbOnDisplay = TRUE;

  alloc_cdik(drawing);

  mask = (XkbStateNotifyMask | XkbNamesNotifyMask | XkbControlsNotifyMask |
          XkbIndicatorMapNotifyMask | XkbNewKeyboardNotifyMask);
  XkbSelectEvents(drawing->display, XkbUseCoreKbd, mask, mask);

  mask = XkbGroupStateMask | XkbModifierStateMask;
  XkbSelectEventDetails(drawing->display, XkbUseCoreKbd, XkbStateNotify, mask,
                        mask);

  mask = (XkbGroupNamesMask | XkbIndicatorNamesMask);
  XkbSelectEventDetails(drawing->display, XkbUseCoreKbd, XkbNamesNotify, mask,
                        mask);
  init_keys_and_doodads(drawing);
  init_colors(drawing);
...
GtkWidget *matekbd_keyboard_drawing_new(void) {
  return GTK_WIDGET(g_object_new(matekbd_keyboard_drawing_get_type(), NULL));
}

static void matekbd_keyboard_drawing_class_init(
    MatekbdKeyboardDrawingClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);
  gtk_widget_class_set_css_name(widget_class, "matekbd-keyboard-drawing");

  klass->bad_keycode = NULL;

  matekbd_keyboard_drawing_signals[BAD_KEYCODE] = g_signal_new(
      "bad-keycode", matekbd_keyboard_drawing_get_type(), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MatekbdKeyboardDrawingClass, bad_keycode), NULL, NULL,
      matekbd_keyboard_drawing_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
}
...
GType matekbd_keyboard_drawing_get_type(void) {
  ...
  if (!matekbd_keyboard_drawing_type) {
    ...
    matekbd_keyboard_drawing_type =
        g_type_register_static(GTK_TYPE_DRAWING_AREA, "MatekbdKeyboardDrawing",
                               &matekbd_keyboard_drawing_info, 0);
  }
...
gboolean matekbd_keyboard_drawing_render(MatekbdKeyboardDrawing *kbdrawing,
  ...
  gtk_style_context_get_background_color(
      style_context, gtk_style_context_get_state(style_context), &dark_color);
  /* make dark background by making regular background darker */
  dark_color.red *= 0.7;
  dark_color.green *= 0.7;
  dark_color.blue *= 0.7;
  ...
  MatekbdKeyboardDrawingRenderContext context = {
      cr, kbdrawing->renderContext->angle, layout, fd, 1, 1, dark_color};

  if (!context_setup_scaling(&context, kbdrawing, width, height, dpi_x,
                             dpi_y)) {
 ...
 */
gboolean matekbd_keyboard_drawing_set_keyboard(MatekbdKeyboardDrawing *drawing,
  ...
  if (names) {
    drawing->xkb = XkbGetKeyboardByName(
        drawing->display, XkbUseCoreKbd, names, 0,
    ...
    drawing->xkb = XkbGetKeyboard(
        drawing->display,
        XkbGBN_GeometryMask | XkbGBN_KeyNamesMask | XkbGBN_OtherNamesMask |
            XkbGBN_SymbolsMask | XkbGBN_IndicatorMapMask,
        XkbUseCoreKbd);
    XkbGetNames(drawing->display, XkbAllNamesMask, drawing->xkb);
    drawing->xkbOnDisplay = TRUE;
  }
...
const gchar *matekbd_keyboard_drawing_get_keycodes(
    MatekbdKeyboardDrawing *drawing) {
...
const gchar *matekbd_keyboard_drawing_get_geometry(
    MatekbdKeyboardDrawing *drawing) {
...
const gchar *matekbd_keyboard_drawing_get_symbols(
    MatekbdKeyboardDrawing *drawing) {
...
const gchar *matekbd_keyboard_drawing_get_types(
    MatekbdKeyboardDrawing *drawing) {
...
const gchar *matekbd_keyboard_drawing_get_compat(
    MatekbdKeyboardDrawing *drawing) {
...
void matekbd_keyboard_drawing_set_track_modifiers(
    MatekbdKeyboardDrawing *drawing, gboolean enable) {
  if (enable) {
    XkbStateRec state;
...
void matekbd_keyboard_drawing_set_track_config(MatekbdKeyboardDrawing *drawing,
                                               gboolean enable) {
  if (enable)
...
void matekbd_keyboard_drawing_set_groups_levels(
    MatekbdKeyboardDrawing *drawing,
...
static void matekbd_keyboard_drawing_begin_print(
...
static void matekbd_keyboard_drawing_draw_page(
  ...
  header = g_strdup_printf(_("Keyboard layout \"%s\"\n"
  ...
  matekbd_keyboard_drawing_render(MATEKBD_KEYBOARD_DRAWING(data->drawing), cr,
                                  layout, 0.0, 0.0, width, height, dpi_x,
                                  dpi_y);
...
void matekbd_keyboard_drawing_print(MatekbdKeyboardDrawing *drawing,
  ...
  g_signal_connect(print, "begin_print",
                   G_CALLBACK(matekbd_keyboard_drawing_begin_print), &data);
  g_signal_connect(print, "draw_page",
                   G_CALLBACK(matekbd_keyboard_drawing_draw_page), &data);
...
static void show_layout_response(GtkWidget *dialog, gint resp) {
  GdkRectangle rect;
  GtkWidget *kbdraw;
  ...
  switch (resp) {
    ...
    case GTK_RESPONSE_PRINT:
      kbdraw = GTK_WIDGET(g_object_get_data(G_OBJECT(dialog), "kbdraw"));
      groupName =
          (const gchar *)g_object_get_data(G_OBJECT(dialog), "groupName");
      matekbd_keyboard_drawing_print(MATEKBD_KEYBOARD_DRAWING(kbdraw),
                                     GTK_WINDOW(dialog),
                                     groupName ? groupName : _("Unknown"));
  }
}
...
GtkWidget *matekbd_keyboard_drawing_new_dialog(gint group, gchar *group_name) {
  ...
  gtk_builder_add_from_resource(
      builder, "/org/mate/peripherals/keyboard/show-layout.ui", &error);
  ...
  snprintf(title, sizeof(title), _("Keyboard Layout \"%s\""), group_name);
  gtk_window_set_title(GTK_WINDOW(dialog), title);
  g_object_set_data_full(G_OBJECT(dialog), "group_name", g_strdup(group_name),
                         g_free);
  ...
  if (xkl_config_rec_get_from_server(xkl_data, engine)) {
    ...
    if (group >= 0 && group < num_layouts && group < num_variants) {
      ...
      if ((p = xkl_data->layouts) != NULL)
        for (i = (gint)num_layouts; --i >= 0;) g_free(*p++);

      if ((p = xkl_data->variants) != NULL)
        for (i = (gint)num_variants; --i >= 0;) g_free(*p++);

      xkl_data->layouts = g_realloc(xkl_data->layouts, sizeof(char *) * 2);
      xkl_data->variants = g_realloc(xkl_data->variants, sizeof(char *) * 2);
      xkl_data->layouts[0] = l;
      xkl_data->variants[0] = v;
      xkl_data->layouts[1] = xkl_data->variants[1] = NULL;
    }
  ...
  gtk_container_add(
      GTK_CONTAINER(gtk_builder_get_object(builder, "preview_vbox")), kbdraw);

  g_object_set_data(G_OBJECT(dialog), "kbdraw", kbdraw);

  g_signal_connect_swapped(dialog, "destroy", G_CALLBACK(g_object_unref),
                           g_object_get_data(G_OBJECT(dialog), "builderData"));
