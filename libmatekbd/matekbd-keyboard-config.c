...
const gchar *MATEKBD_KEYBOARD_CONFIG_ACTIVE[] = {
    MATEKBD_KEYBOARD_CONFIG_KEY_MODEL, MATEKBD_KEYBOARD_CONFIG_KEY_LAYOUTS,
...
static gboolean matekbd_strv_equal(gchar **l1, gchar **l2) {
  if (l1 == l2) return TRUE;
  if (l1 == NULL) return g_strv_length(l2) == 0;
  if (l2 == NULL) return g_strv_length(l1) == 0;

  while ((*l1 != NULL) && (*l2 != NULL)) {
    if (*l1 != *l2) {
      if (*l1 && *l2) {
        if (g_ascii_strcasecmp(*l1, *l2)) return FALSE;
      } else
        return FALSE;
    }
...
gboolean matekbd_keyboard_config_get_lv_descriptions(
  ...
  if (litem == NULL) litem = xkl_config_item_new();
  if (vitem == NULL) vitem = xkl_config_item_new();
  ...
  if (variant_name != NULL) {
    ...
    g_snprintf(vitem->name, sizeof vitem->name, "%s", variant_name);
    if (xkl_config_registry_find_variant(config_registry, layout_name, vitem)) {
 ...
 */
const gchar *matekbd_keyboard_config_merge_items(const gchar *parent,
                                                 const gchar *child) {
  static gchar buffer[XKL_MAX_CI_NAME_LENGTH * 2 - 1];
  *buffer = '\0';
  if (parent != NULL) {
    if (strlen(parent) >= XKL_MAX_CI_NAME_LENGTH) return NULL;
    strcat(buffer, parent);
  }
  if (child != NULL && *child != 0) {
    if (strlen(child) >= XKL_MAX_CI_NAME_LENGTH) return NULL;
    strcat(buffer, "\t");
    strcat(buffer, child);
  }
  return buffer;
...
gboolean matekbd_keyboard_config_split_items(const gchar *merged,
                                             gchar **parent, gchar **child) {
  ...
  *parent = *child = NULL;
  ...
  if (pos == NULL) {
    ...
    if (strlen(pos + 1) >= XKL_MAX_CI_NAME_LENGTH) return FALSE;
    strcpy(*child = cbuffer, pos + 1);
  }
  if (plen >= XKL_MAX_CI_NAME_LENGTH) return FALSE;
...
static void matekbd_keyboard_config_copy_from_xkl_config(
  ...
  if (pdata->layouts != NULL) {
    ...
    i = 0;
    while (*p != NULL) {
      const gchar *full_layout = matekbd_keyboard_config_merge_items(*p, *p1);
      xkl_debug(150, "Loaded Kbd layout (with variant): [%s]\n", full_layout);
...
static void matekbd_keyboard_config_copy_to_xkl_config(
    MatekbdKeyboardConfig *kbd_config, XklConfigRec *pdata) {
  int i;
  guint num_layouts, num_options;
  pdata->model =
      (kbd_config->model == NULL) ? NULL : g_strdup(kbd_config->model);

  num_layouts = (kbd_config->layouts_variants == NULL)
                    ? 0
                    : g_strv_length(kbd_config->layouts_variants);
  num_options =
      (kbd_config->options == NULL) ? 0 : g_strv_length(kbd_config->options);

  xkl_debug(150, "Taking %d layouts\n", num_layouts);
  if (num_layouts != 0) {
    gchar **the_layout_variant = kbd_config->layouts_variants;
    char **p1 = pdata->layouts = g_new0(char *, (gsize)(num_layouts + 1));
    char **p2 = pdata->variants = g_new0(char *, (gsize)(num_layouts + 1));
    for (i = (int)num_layouts; --i >= 0;) {
      char *layout, *variant;
      if (matekbd_keyboard_config_split_items(*the_layout_variant, &layout,
                                              &variant) &&
          variant != NULL) {
  ...
  if (num_options != 0) {
    ...
    char **p = pdata->options = g_new0(char *, num_options + 1);
    for (i = (int)num_options; --i >= 0;) {
      char *group, *option;
      if (matekbd_keyboard_config_split_items(*the_option, &group, &option) &&
          option != NULL)
        *(p++) = g_strdup(option);
      else {
        *(p++) = g_strdup("");
...
static void matekbd_keyboard_config_load_params(
    MatekbdKeyboardConfig *kbd_config, const gchar *param_names[]) {
  gchar *pc;
...
static void matekbd_keyboard_config_save_params(
    MatekbdKeyboardConfig *kbd_config, const gchar *param_names[]) {
  gchar **pl;
  ...
  if (kbd_config->model)
    ...
    g_settings_set_string(kbd_config->settings, param_names[0], NULL);
  ...
  if (kbd_config->layouts_variants) {
    ...
    g_settings_set_strv(kbd_config->settings, param_names[1],
                        (const gchar *const *)kbd_config->layouts_variants);
  } else {
    xkl_debug(150, "Saved Kbd layouts: []\n");
    g_settings_set_strv(kbd_config->settings, param_names[1], NULL);
  }
  ...
  if (kbd_config->options) {
    ...
    g_settings_set_strv(kbd_config->settings, param_names[2],
                        (const gchar *const *)kbd_config->options);
  } else {
    xkl_debug(150, "Saved Kbd options: []\n");
    g_settings_set_strv(kbd_config->settings, param_names[2], NULL);
  }
}
 ...
 */
void matekbd_keyboard_config_init(MatekbdKeyboardConfig *kbd_config,
                                  XklEngine *engine) {
...
void matekbd_keyboard_config_term(MatekbdKeyboardConfig *kbd_config) {
  matekbd_keyboard_config_model_set(kbd_config, NULL);
...
void matekbd_keyboard_config_load_from_gsettings(
    MatekbdKeyboardConfig *kbd_config,
    MatekbdKeyboardConfig *kbd_config_default) {
  ...
  if (kbd_config_default != NULL) {
    if (kbd_config->model == NULL)
      kbd_config->model = g_strdup(kbd_config_default->model);

    if (kbd_config->layouts_variants == NULL) {
      kbd_config->layouts_variants =
          g_strdupv(kbd_config_default->layouts_variants);
    }

    if (kbd_config->options == NULL) {
      kbd_config->options = g_strdupv(kbd_config_default->options);
    }
  }
}

void matekbd_keyboard_config_load_from_x_current(
    MatekbdKeyboardConfig *kbd_config, XklConfigRec *data) {
  gboolean own_data = data == NULL;
  xkl_debug(150, "Copying config from X(current)\n");
  if (own_data) data = xkl_config_rec_new();
  if (xkl_config_rec_get_from_server(data, kbd_config->engine))
    matekbd_keyboard_config_copy_from_xkl_config(kbd_config, data);
  else
    xkl_debug(150, "Could not load keyboard config from server: [%s]\n",
              xkl_get_last_error());
  if (own_data) g_object_unref(G_OBJECT(data));
}

void matekbd_keyboard_config_load_from_x_initial(
    MatekbdKeyboardConfig *kbd_config, XklConfigRec *data) {
  gboolean own_data = data == NULL;
  ...
  if (xkl_config_rec_get_from_backup(data, kbd_config->engine))
    ...
    xkl_debug(150, "Could not load keyboard config from backup: [%s]\n",
              xkl_get_last_error());
  if (own_data) g_object_unref(G_OBJECT(data));
}

static gboolean matekbd_keyboard_config_options_equals(
    MatekbdKeyboardConfig *kbd_config1, MatekbdKeyboardConfig *kbd_config2) {
  guint num_options, num_options2;

  num_options =
      (kbd_config1->options == NULL) ? 0 : g_strv_length(kbd_config1->options);
  num_options2 =
      (kbd_config2->options == NULL) ? 0 : g_strv_length(kbd_config2->options);

  if (num_options != num_options2) return False;
  ...
  if (num_options != 0) {
    ...
    for (i = 0; i < num_options; i++) {
      ...
      if (!matekbd_keyboard_config_split_items(kbd_config1->options[i], &group1,
                                               &option1))
        continue;
      ...
      for (j = 0; j < num_options && !are_equal; j++) {
        if (matekbd_keyboard_config_split_items(kbd_config2->options[j],
                                                &group2, &option2)) {
...
gboolean matekbd_keyboard_config_equals(MatekbdKeyboardConfig *kbd_config1,
                                        MatekbdKeyboardConfig *kbd_config2) {
  if (kbd_config1 == kbd_config2) return True;
  if ((kbd_config1->model != kbd_config2->model) &&
      (kbd_config1->model != NULL) && (kbd_config2->model != NULL) &&
  ...
  if (!matekbd_keyboard_config_options_equals(kbd_config1, kbd_config2))
    return False;

  return True;
}

void matekbd_keyboard_config_save_to_gsettings(
    MatekbdKeyboardConfig *kbd_config) {
...
void matekbd_keyboard_config_model_set(MatekbdKeyboardConfig *kbd_config,
                                       const gchar *model_name) {
  if (kbd_config->model != NULL) g_free(kbd_config->model);
  kbd_config->model = (model_name == NULL || model_name[0] == '\0')
                          ? NULL
                          : g_strdup(model_name);
}

void matekbd_keyboard_config_options_set(MatekbdKeyboardConfig *kbd_config,
                                         gint idx, const gchar *group_name,
                                         const gchar *option_name) {
  const gchar *merged;
  if (group_name == NULL || option_name == NULL) return;
  merged = matekbd_keyboard_config_merge_items(group_name, option_name);
  if (merged == NULL) return;
  kbd_config->options[idx] = g_strdup(merged);
}

gboolean matekbd_keyboard_config_options_is_set(
  ...
  while (p && *p) {
    if (!g_ascii_strcasecmp(merged, *p++)) return TRUE;
  }
  return FALSE;
}

gboolean matekbd_keyboard_config_activate(MatekbdKeyboardConfig *kbd_config) {
  gboolean rv;
 ...
 */
void matekbd_keyboard_config_start_listen(MatekbdKeyboardConfig *kbd_config,
                                          GCallback func, gpointer user_data) {
  kbd_config->config_listener_id =
      g_signal_connect(kbd_config->settings, "changed", func, user_data);
}

void matekbd_keyboard_config_stop_listen(MatekbdKeyboardConfig *kbd_config) {
#if GLIB_CHECK_VERSION(2, 62, 0)
  g_clear_signal_handler(&kbd_config->config_listener_id, kbd_config->settings);
#else
...
gboolean matekbd_keyboard_config_get_descriptions(
    XklConfigRegistry *config_registry, const gchar *name,
    gchar **layout_short_descr, gchar **layout_descr,
    gchar **variant_short_descr, gchar **variant_descr) {
  char *layout_name = NULL, *variant_name = NULL;
  if (!matekbd_keyboard_config_split_items(name, &layout_name, &variant_name))
    return FALSE;
  return matekbd_keyboard_config_get_lv_descriptions(
      config_registry, layout_name, variant_name, layout_short_descr,
      layout_descr, variant_short_descr, variant_descr);
}

const gchar *matekbd_keyboard_config_format_full_layout(
    const gchar *layout_descr, const gchar *variant_descr) {
  static gchar full_descr[XKL_MAX_CI_DESC_LENGTH * 2];
  if (variant_descr == NULL || variant_descr[0] == 0)
    g_snprintf(full_descr, sizeof(full_descr), "%s", layout_descr);
  else
    g_snprintf(full_descr, sizeof(full_descr), "%s %s", layout_descr,
               variant_descr);
  return full_descr;
}

gchar *matekbd_keyboard_config_to_string(const MatekbdKeyboardConfig *config) {
  ...
  if (config->layouts_variants) {
    ...
    for (iter = config->layouts_variants, count = 0; *iter; iter++, ++count) {
      if (buffer->len) g_string_append(buffer, " ");
    ...
    layouts = g_strdup_printf(
        ngettext("layout \"%s\"", "layouts \"%s\"", count), buffer->str);
    g_string_truncate(buffer, 0);
  }
  if (config->options) {
    /* g_slist_length is "expensive", so we determinate the length on the fly */
    for (iter = config->options, count = 0; *iter; iter++, ++count) {
      if (buffer->len) g_string_append(buffer, " ");
    ...
    options = g_strdup_printf(
        ngettext("option \"%s\"", "options \"%s\"", count), buffer->str);
    g_string_truncate(buffer, 0);
  }

  g_string_free(buffer, TRUE);

  result = g_strdup_printf(_("model \"%s\", %s and %s"), config->model,
 ...
 */
gchar **matekbd_keyboard_config_add_default_switch_option_if_necessary(
    gchar **layouts_list, gchar **options_list, gboolean *was_appended) {
  *was_appended = FALSE;
  if (g_strv_length(layouts_list) >= 2) {
    ...
    if (*options_list != NULL) {
      ...
      while (*option != NULL) {
        char *g, *o;
        if (matekbd_keyboard_config_split_items(*option, &g, &o)) {
    ...
    if (!any_switcher) {
      const gchar *id = matekbd_keyboard_config_merge_items(
          GROUP_SWITCHERS_GROUP, DEFAULT_GROUP_SWITCH);
      options_list = matekbd_strv_append(options_list, g_strdup(id));
      *was_appended = TRUE;
    }
  }
  return options_list;
