...
static gboolean matekbd_desktop_config_get_lv_descriptions(
    MatekbdDesktopConfig *config, XklConfigRegistry *registry,
    const gchar **layout_ids, const gchar **variant_ids,
    gchar ***short_layout_descriptions, gchar ***long_layout_descriptions,
    gchar ***short_variant_descriptions, gchar ***long_variant_descriptions) {
  const gchar **pl, **pv;
  guint total_layouts;
 ...
 */
void matekbd_desktop_config_init(MatekbdDesktopConfig *config,
                                 XklEngine *engine) {
...
void matekbd_desktop_config_term(MatekbdDesktopConfig *config) {
  g_object_unref(config->settings);
  config->settings = NULL;
}

void matekbd_desktop_config_load_from_gsettings(MatekbdDesktopConfig *config) {
  config->group_per_app = g_settings_get_boolean(
      config->settings, MATEKBD_DESKTOP_CONFIG_KEY_GROUP_PER_WINDOW);
  xkl_debug(150, "group_per_app: %d\n", config->group_per_app);

  config->handle_indicators = g_settings_get_boolean(
      config->settings, MATEKBD_DESKTOP_CONFIG_KEY_HANDLE_INDICATORS);
  xkl_debug(150, "handle_indicators: %d\n", config->handle_indicators);

  config->layout_names_as_group_names = g_settings_get_boolean(
      config->settings, MATEKBD_DESKTOP_CONFIG_KEY_LAYOUT_NAMES_AS_GROUP_NAMES);
  xkl_debug(150, "layout_names_as_group_names: %d\n",
            config->layout_names_as_group_names);

  config->load_extra_items = g_settings_get_boolean(
      config->settings, MATEKBD_DESKTOP_CONFIG_KEY_LOAD_EXTRA_ITEMS);
  xkl_debug(150, "load_extra_items: %d\n", config->load_extra_items);

  config->default_group = g_settings_get_int(
      config->settings, MATEKBD_DESKTOP_CONFIG_KEY_DEFAULT_GROUP);

  if (config->default_group < -1 ||
      config->default_group >= xkl_engine_get_max_num_groups(config->engine))
    config->default_group = -1;
  xkl_debug(150, "default_group: %d\n", config->default_group);
}

void matekbd_desktop_config_save_to_gsettings(MatekbdDesktopConfig *config) {
  ...
  g_settings_set_boolean(config->settings,
                         ...
                         config->load_extra_items);
  g_settings_set_int(config->settings, MATEKBD_DESKTOP_CONFIG_KEY_DEFAULT_GROUP,
                     config->default_group);

  g_settings_apply(config->settings);
}

gboolean matekbd_desktop_config_activate(MatekbdDesktopConfig *config) {
  gboolean rv = TRUE;

  xkl_engine_set_group_per_toplevel_window(config->engine,
                                           config->group_per_app);
  xkl_engine_set_indicators_handling(config->engine, config->handle_indicators);
  xkl_engine_set_default_group(config->engine, config->default_group);

  return rv;
}

void matekbd_desktop_config_lock_next_group(MatekbdDesktopConfig *config) {
  int group = xkl_engine_get_next_group(config->engine);
  xkl_engine_lock_group(config->engine, group);
}

void matekbd_desktop_config_lock_prev_group(MatekbdDesktopConfig *config) {
  int group = xkl_engine_get_prev_group(config->engine);
  xkl_engine_lock_group(config->engine, group);
}

void matekbd_desktop_config_restore_group(MatekbdDesktopConfig *config) {
 ...
 */
void matekbd_desktop_config_start_listen(MatekbdDesktopConfig *config,
                                         GCallback func, gpointer user_data) {
  config->config_listener_id =
      g_signal_connect(config->settings, "changed", func, user_data);
}

void matekbd_desktop_config_stop_listen(MatekbdDesktopConfig *config) {
#if GLIB_CHECK_VERSION(2, 62, 0)
  g_clear_signal_handler(&config->config_listener_id, config->settings);
#else
  if (config->config_listener_id != 0) {
    g_signal_handler_disconnect(config->settings, config->config_listener_id);
    config->config_listener_id = 0;
  }
#endif
}

gboolean matekbd_desktop_config_load_group_descriptions(
    MatekbdDesktopConfig *config, XklConfigRegistry *registry,
    const gchar **layout_ids, const gchar **variant_ids,
  ...
  if (!matekbd_desktop_config_get_lv_descriptions(
          config, registry, layout_ids, variant_ids, &sld, &lld, &svd, &lvd)) {
    return False;
  }

  total_descriptions = g_strv_length(sld);

  *short_group_names = psgn = g_new0(gchar *, (gsize)(total_descriptions + 1));
  *full_group_names = pfgn = g_new0(gchar *, (gsize)(total_descriptions + 1));
  ...
  while (plld != NULL && *plld != NULL) {
    ...
    *psgn++ = g_strdup(sd);
    *pfgn++ =
        g_strdup(matekbd_keyboard_config_format_full_layout(*plld++, *plvd++));
  }
  g_strfreev(sld);
  g_strfreev(lld);
  g_strfreev(svd);
