...
static void matekbd_status_global_init(void);
static void matekbd_status_global_term(void);
static GdkPixbuf *matekbd_status_prepare_drawing(MatekbdStatus *gki,
                                                 guint group);
static void matekbd_status_set_current_page_for_group(MatekbdStatus *gki,
                                                      guint group);
static void matekbd_status_set_current_page(MatekbdStatus *gki);
static void matekbd_status_global_cleanup(MatekbdStatus *gki);
static void matekbd_status_global_fill(MatekbdStatus *gki);
static void matekbd_status_set_tooltips(MatekbdStatus *gki, const char *str);

void matekbd_status_set_tooltips(MatekbdStatus *gki, const char *str) {
  g_assert(str == NULL || g_utf8_validate(str, -1, NULL));

  gtk_status_icon_set_tooltip_text(GTK_STATUS_ICON(gki), str);
}

void matekbd_status_global_cleanup(MatekbdStatus *gki) {
  while (globals.icons) {
    if (globals.icons->data) g_object_unref(G_OBJECT(globals.icons->data));
    globals.icons = g_slist_delete_link(globals.icons, globals.icons);
  }
}

void matekbd_status_global_fill(MatekbdStatus *gki) {
  guint grp;
...
static void matekbd_status_activate(MatekbdStatus *gki) {
  xkl_debug(150, "Mouse button pressed on applet\n");
  matekbd_desktop_config_lock_next_group(&globals.cfg);
}

/* hackish xref */
extern gchar *matekbd_indicator_extract_layout_name(
    guint group, XklEngine *engine, MatekbdKeyboardConfig *kbd_cfg,
    gchar **short_group_names, gchar **full_group_names);

extern gchar *matekbd_indicator_create_label_title(guint group,
                                                   GHashTable **ln2cnt_map,
                                                   gchar *layout_name);

static void matekbd_status_render_cairo(cairo_t *cr, guint group) {
  double r, g, b;
  ...
  if (globals.ind_cfg.background_color != NULL &&
    ...
    if (sscanf(globals.ind_cfg.background_color, "%lg %lg %lg", &r, &g, &b) ==
        3) {
      cairo_set_source_rgb(cr, r, g, b);
      cairo_rectangle(cr, 0, 0, globals.current_width, globals.current_height);
      cairo_fill(cr);
    }
  }
  ...
  if (globals.ind_cfg.foreground_color != NULL &&
    ...
    if (sscanf(globals.ind_cfg.foreground_color, "%lg %lg %lg", &r, &g, &b) ==
        3) {
      cairo_set_source_rgb(cr, r, g, b);
    }
  }
  ...
  if (screen_res > 0) pango_cairo_context_set_resolution(pcc, screen_res);

  fo = cairo_font_options_copy(
      gdk_screen_get_font_options(gdk_screen_get_default()));
  /* SUBPIXEL antialiasing gives bad results on in-memory images */
  if (cairo_font_options_get_antialias(fo) == CAIRO_ANTIALIAS_SUBPIXEL)
    cairo_font_options_set_antialias(fo, CAIRO_ANTIALIAS_GRAY);
  pango_cairo_context_set_font_options(pcc, fo);

  pl = pango_layout_new(pcc);

  layout_name = matekbd_indicator_extract_layout_name(
      group, globals.engine, &globals.kbd_cfg, globals.short_group_names,
      globals.full_group_names);
  lbl_title =
      matekbd_indicator_create_label_title(group, &ln2cnt_map, layout_name);
  ...
  cairo_move_to(cr, (globals.current_width - lwidth / PANGO_SCALE) / 2,
                (globals.current_height - lheight / PANGO_SCALE) / 2);
...
static void convert_bgra_to_rgba(guint8 const *src, guint8 *dst, int width,
...
static GdkPixbuf *matekbd_status_prepare_drawing(MatekbdStatus *gki,
  ...
  if (globals.ind_cfg.show_flags) {
    ...
    xkl_debug(150, "Image %u[%s] loaded -> %p[%dx%d], alpha: %d\n", group,
              image_filename, image, gdk_pixbuf_get_width(image),
              gdk_pixbuf_get_height(image), gdk_pixbuf_get_has_alpha(image));

    return image;
  } else {
    cairo_surface_t *cs = cairo_image_surface_create(
        CAIRO_FORMAT_ARGB32, globals.current_width, globals.current_height);
    ...
    convert_bgra_to_rgba(cairo_data, pixbuf_data, globals.current_width,
                         globals.current_height, globals.real_width);

    cairo_surface_destroy(cs);

    image = gdk_pixbuf_new_from_data(pixbuf_data, GDK_COLORSPACE_RGB, TRUE, 8,
                                     globals.real_width, globals.current_height,
                                     globals.real_width * 4,
                                     (GdkPixbufDestroyNotify)g_free, NULL);
    xkl_debug(150, "Image %u created -> %p[%dx%d], alpha: %d\n", group, image,
...
static void matekbd_status_update_tooltips(MatekbdStatus *gki) {
...
void matekbd_status_reinit_ui(MatekbdStatus *gki) {
...
static void matekbd_status_cfg_changed(GSettings *settings, gchar *key,
                                       gpointer user_data) {
  xkl_debug(100, "General configuration changed in settings - reiniting...\n");
  matekbd_desktop_config_load_from_gsettings(&globals.cfg);
  matekbd_desktop_config_activate(&globals.cfg);
  ForAllIndicators() { matekbd_status_reinit_ui(gki); }
  NextIndicator();
}

/* Should be called once for all widgets */
static void matekbd_status_ind_cfg_changed(GSettings *settings, gchar *key,
                                           gpointer user_data) {
  xkl_debug(100, "Applet configuration changed in settings - reiniting...\n");
...
static void matekbd_status_load_group_names(const gchar **layout_ids,
  ...
  if (!matekbd_desktop_config_load_group_descriptions(
    ...
    globals.full_group_names = g_new0(char *, (gsize)(total_groups + 1));
...
/* Should be called once for all widgets */
static void matekbd_status_kbd_cfg_callback(MatekbdStatus *gki) {
  XklConfigRec *xklrec = xkl_config_rec_new();
  xkl_debug(100, "XKB configuration changed on X Server - reiniting...\n");

  matekbd_keyboard_config_load_from_x_current(&globals.kbd_cfg, xklrec);
...
static void matekbd_status_state_callback(XklEngine *engine,
...
void matekbd_status_set_current_page(MatekbdStatus *gki) {
  XklState *cur_state;
...
void matekbd_status_set_current_page_for_group(MatekbdStatus *gki,
                                               guint group) {
  xkl_debug(200, "Revalidating for group %d\n", group);

  gtk_status_icon_set_from_pixbuf(
      GTK_STATUS_ICON(gki), GDK_PIXBUF(g_slist_nth_data(globals.icons, group)));

  matekbd_status_update_tooltips(gki);
}

/* Should be called once for all widgets */
static GdkFilterReturn matekbd_status_filter_x_evt(GdkXEvent *xev,
                                                   GdkEvent *event) {
  ...
  switch (xevent->type) {
    case ReparentNotify: {
      ...
      ForAllIndicators() {
        guint32 xid = gtk_status_icon_get_x11_window_id(GTK_STATUS_ICON(gki));

        /* compare the indicator's parent window with the even window */
        if (xid == rne->window) {
          /* if so - make it transparent... */
          xkl_engine_set_window_transparent(globals.engine, rne->window, TRUE);
        }
      }
      NextIndicator()
    } break;
...
/* Should be called once for all widgets */
static void matekbd_status_start_listen(void) {
  gdk_window_add_filter(NULL, (GdkFilterFunc)matekbd_status_filter_x_evt, NULL);
  gdk_window_add_filter(gdk_get_default_root_window(),
                        (GdkFilterFunc)matekbd_status_filter_x_evt, NULL);

  xkl_engine_start_listen(globals.engine, XKLL_TRACK_KEYBOARD_STATE);
}

/* Should be called once for all widgets */
static void matekbd_status_stop_listen(void) {
  xkl_engine_stop_listen(globals.engine, XKLL_TRACK_KEYBOARD_STATE);

  gdk_window_remove_filter(NULL, (GdkFilterFunc)matekbd_status_filter_x_evt,
                           NULL);
  gdk_window_remove_filter(gdk_get_default_root_window(),
                           (GdkFilterFunc)matekbd_status_filter_x_evt, NULL);
}

static void matekbd_status_size_changed(MatekbdStatus *gki, gint size) {
...
static void matekbd_status_theme_changed(GtkSettings *settings,
                                         GParamSpec *pspec,
                                         MatekbdStatus *gki) {
...
static void matekbd_status_init(MatekbdStatus *gki) {
  int i;

  if (!g_slist_length(globals.widget_instances)) matekbd_status_global_init();
  ...
  if (globals.engine == NULL) {
    matekbd_status_set_tooltips(gki, _("XKB initialization error"));
    return;
  }
  ...
  globals.widget_instances = g_slist_append(globals.widget_instances, gki);

  g_signal_connect(gki, "size-changed", G_CALLBACK(matekbd_status_size_changed),
                   NULL);
  g_signal_connect(gki, "activate", G_CALLBACK(matekbd_status_activate), NULL);

  for (i = G_N_ELEMENTS(settings_signal_names); --i >= 0;)
    gki->priv->settings_signal_handlers[i] = g_signal_connect_after(
        gtk_settings_get_default(), settings_signal_names[i],
        G_CALLBACK(matekbd_status_theme_changed), gki);
}

static void matekbd_status_finalize(GObject *obj) {
  int i;
  ...
  xkl_debug(100, "The instance of mate-kbd-status successfully finalized\n");

  g_free(gki->priv);

  G_OBJECT_CLASS(matekbd_status_parent_class)->finalize(obj);

  if (!g_slist_length(globals.widget_instances)) matekbd_status_global_term();
}
...
static void matekbd_status_global_term(void) {
  ...
  if ((globals.state_changed_handler != 0) &&
    ...
    g_clear_signal_handler(&globals.state_changed_handler, globals.engine);
...
static void matekbd_status_class_init(MatekbdStatusClass *klass) {
...
static void matekbd_status_global_init(void) {
  ...
  globals.state_changed_handler =
      g_signal_connect(globals.engine, "X-state-changed",
                       G_CALLBACK(matekbd_status_state_callback), NULL);
  globals.config_changed_handler =
      g_signal_connect(globals.engine, "X-config-changed",
                       G_CALLBACK(matekbd_status_kbd_cfg_callback), NULL);
  ...
  matekbd_desktop_config_start_listen(
      &globals.cfg, (GCallback)matekbd_status_cfg_changed, NULL);
  matekbd_indicator_config_start_listen(
      &globals.ind_cfg, (GCallback)matekbd_status_ind_cfg_changed, NULL);
...
GtkStatusIcon *matekbd_status_new(void) {
  return GTK_STATUS_ICON(g_object_new(matekbd_status_get_type(), NULL));
}
 ...
 */
XklEngine *matekbd_status_get_xkl_engine() { return globals.engine; }
 ...
 */
gchar **matekbd_status_get_group_names() { return globals.full_group_names; }

gchar *matekbd_status_get_image_filename(guint group) {
  if (!globals.ind_cfg.show_flags) return NULL;
  return matekbd_indicator_config_get_images_file(&globals.ind_cfg,
                                                  &globals.kbd_cfg, group);
}

void matekbd_status_set_angle(MatekbdStatus *gki, gdouble angle) {
